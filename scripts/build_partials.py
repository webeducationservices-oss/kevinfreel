...
def main() -> int:
    ...
    # neighborhoods/ is generated by build_neighborhoods.py, but its pages carry
    # the same BEGIN_/END_ markers and must be listed here too. Left out, a nav
    # edit propagated to these 156 pages and silently skipped those 52, so the
    # site navigated differently depending on which page you landed on. That is
    # the same drift that reverted the trailing-slash fix three times.
    targets = (
        sorted(ROOT.glob("*.html"))
        + sorted((ROOT / "blog").glob("*.html"))
        + sorted((ROOT / "listings").glob("*.html"))
        + sorted((ROOT / "neighborhoods").glob("*.html"))
    )
